...
Ts = 36000; % Tempo de amostragem

% Cria edpaço de estados com atrasos
plant_with_delays = ss(A, B, C, D, Ts, 'InputDelay', [2; 1], 'OutputDelay', [3; 5]);

% Absorve os atrasos para criar o modelo que o MPC
...
PredictionHorizon = 15;  % Horizonte de predição
ControlHorizon = 5;      % Horizonte de controle

% Criar o MPC usando o modelo com atrasos
...
mpc_controller.Weights.MVRate = [0.5 0.5]; 
mpc_controller.Weights.OV = [1 1.2];      

% Restrições operacionais (limites físicos do sistema).
mpc_controller.MV(1).Min = 0; mpc_controller.MV(1).Max = 5; 
mpc_controller.MV(2).Min = 0; mpc_controller.MV(2).Max = 4; 
mpc_controller.OV(1).Min = 0.5; mpc_controller.OV(1).Max = 2.5; 
mpc_controller.OV(2).Min = 0.8; mpc_controller.OV(2).Max = 2.0;

% =========================================================================
% PASSO 3: SIMULAR O SISTEMA USANDO UM LOOP MANUAL
% =========================================================================

Tsim_steps = 60; % imulação em número de passos
r = [1.8; 1.5];  % Setpoints

% --- Inicialização da Simulação ---
x_plant = [1; 1]; % Estado inicial da planta real ([nivel1; nivel2])
x_mpc = mpcstate(mpc_controller); % Inicializa o estado interno do MPC
...
for k = 1:Tsim_steps
    ...
    % mpcmove(controlador, estado_controlador, saida_medida, setpoint)
    u = mpcmove(mpc_controller, x_mpc, y_meas, r);
    
    % 3. Aplicar a ação de controle
